...
interface Props {
  ...
  continent: string;
  sites: DiveSite[];
  nearbyCities: { name: string; slug: string; count: number }[];
}
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          ...
          <Box sx={{
            ...
            <Stack direction="row" alignItems="center" spacing={0.5} mb={{ xs: 2, md: 3 }}>
              ...
              <Typography
                component={Link}
                href={`/dive-sites/country/${countrySlug}`}
                sx={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.75rem', textDecoration: 'none', '&:hover': { color: 'rgba(255,255,255,0.8)' }, transition: 'color 0.15s' }}
              >
                {countryName}
              </Typography>
              <Typography sx={{ color: 'rgba(255,255,255,0.25)', fontSize: '0.75rem' }}>/</Typography>
              <Typography sx={{ color: 'rgba(255,255,255,0.75)', fontSize: '0.75rem', fontWeight: 500 }}>{cityName}</Typography>
            </Stack>
          ...
          <Box sx={{ position: 'sticky', top: 0, zIndex: 20, bgcolor: 'background.paper', borderBottom: '1px solid', borderColor: 'divider' }}>
            ...
            <Stack direction="row" spacing={1.5} alignItems="center"
              ...
              <TextField
                ...
                size="small"
                fullWidth
                inputProps={{ 'aria-label': `Search dive sites in ${cityName}` }}
                sx={{
                  flex: 1,
                  maxWidth: { sm: 400 },
            ...
            {/* Row 3: nearby cities (horizontal scroll) */}
            {nearbyCities.length > 0 && (
              <Box sx={{ borderTop: '1px solid', borderColor: 'divider', position: 'relative' }}>
                <Box sx={{
                  ...
                  {nearbyCities.map(({ name, slug, count }) => (
                    <Chip
                      key={slug}
                      component={Link}
          ...
          <Box sx={{ px: { xs: 2, sm: 3, md: 4 }, py: 3 }}>
            <Typography variant="body2" color="text.secondary" mb={2.5} sx={{ display: { sm: 'none' } }}>
              {filtered.length} {filtered.length === 1 ? 'site' : 'sites'} in {cityName}
            </Typography>
